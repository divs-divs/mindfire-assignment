...
class AddAirport extends Component {
    ...
    async handleSubmit(event) {
        if (validate.validate(this.validator)) {
            if (parseInt(this.state.fuel_available) > parseInt(this.state.fuel_capacity)) {
                toastservice.info("Fuel available can not be more than Fuel capacity");
            }
            else {
                $.ajax({
